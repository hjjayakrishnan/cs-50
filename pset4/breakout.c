...
int main(void)
{
    ...
    int max = 2;
    int min = 1;
    double xVel=min + (int)(drand48()*2)%(max-min);
    ...
    while (lives > 0 && bricks > 0)
    {
        ...
        if (event != NULL)
        {
            ...
            if (getEventType(event) == MOUSE_MOVED)
            {
                ...
                setLocation(paddle,getX(event),500);
                
                if(getX(paddle)>=(WIDTH-PADDLEw))
                
                setLocation(paddle,WIDTH-PADDLEw,500);
...
string color[] ={"BLUE","CYAN","MAGENTA","RED","YELLOW"};

    for(int i =0;i< ROWS ; i++)
    {
        for(int j =0;j<COLS ;j++)
        {
            Brick[i][j] = newGRect(j*(WIDTH/COLS)+2,i*BRICKh+OFFSET,WIDTH/COLS-4,BRICKh-4);
...
GRect initPaddle(GWindow window)
{
    GRect paddle = newGRect(WIDTH/2 - PADDLEw/2,500, PADDLEw,PADDLEh);
